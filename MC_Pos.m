function [Z, best_mAP, best_mAUC] = MC_Pos(W, Y_gt, option, para)
% function [best_mAP, best_mAUC, Z] = MC_Pos(W, Y_gt, lambda, red_factor, mu_thresh, shldNrm, g, bin_type)
% -------------------------------------------------------------------------
...
last_err = Inf;
% -------------------------------------------------------------------------
% Main loop of the Fixed Point Continuation algorithm for solving MC-Pos
% -------------------------------------------------------------------------
for mu = mus 
	err_diff = Inf;
	...
	if (r ~= 0)
        ...
        if tst_mAUC > best_mAUC
            ...
            fprintf('MCPos: mu = %d, lambda = %f, mAUC = %f\n\n', mu, para.lambda, tst_mAUC);
        end
        % % [old_auroc] = auroc2(Y(:,trainsize+1:end), Y_gt, Y(:,1:trainsize));
        % [tst_auroc] = auroc(Y(:,trainsize+1:end), Y_gt);
        % if tst_auroc > best_auroc
        %     best_auroc = tst_auroc;
        %     fprintf('MCPos: mu = %d, lambda = %f, AUROC = %f\n', mu, lambda, tst_auroc);
        % end
    end
end
